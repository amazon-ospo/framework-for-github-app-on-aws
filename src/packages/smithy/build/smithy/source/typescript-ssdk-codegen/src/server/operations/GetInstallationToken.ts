...
export const getGetInstallationTokenHandler = <Context>(operation: __Operation<GetInstallationTokenServerInput, GetInstallationTokenServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  const customizer: __ValidationCustomizer<"GetInstallationToken"> = (ctx, failures) => {
    if (!failures) {
      return undefined;
    }
    return {
