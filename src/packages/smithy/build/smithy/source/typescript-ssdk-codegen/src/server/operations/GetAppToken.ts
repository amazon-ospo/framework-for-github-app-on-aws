...
export const getGetAppTokenHandler = <Context>(operation: __Operation<GetAppTokenServerInput, GetAppTokenServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  const customizer: __ValidationCustomizer<"GetAppToken"> = (ctx, failures) => {
    if (!failures) {
      return undefined;
    }
    return {
